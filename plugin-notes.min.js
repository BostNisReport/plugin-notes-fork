...
function delete_plugin_note(plugin_name) {
	...
	jQuery.confirm({
		title: '',
		content: i18n_plugin_notes.confirm_delete,
		buttons: {
			confirm: function () {
				//jQuery.alert(i18n_plugin_notes.confirm_delete);
				var note_elements = get_plugin_note_elements(plugin_name);
				note_elements.box.find('.waiting').show();
				note_elements.form.input.val('');
				save_plugin_note( plugin_name );
...
function save_plugin_note(plugin_name) {
    ...
    jQuery.ajax({
        ...
        success: function(xml) {
            plugin_note_saved(xml, note_elements)
        },
        error: function(xml) {
            plugin_note_error(xml, note_elements)
        }
    });
    return false
}
