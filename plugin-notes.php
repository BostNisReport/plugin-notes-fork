...
if( !class_exists('plugin_notes')) {
	...
	class plugin_notes {
		...
		function _add_plugin_note ( $note = null, $plugin_data, $plugin_file, $echo = true ) {
			...
			$file=WP_PLUGIN_DIR .'/'. esc_js( $plugin_safe_name ).'.md';
			//$files = "plugin-slug-readme.md";
			
		
			if(file_exists(WP_PLUGIN_DIR . '/'.esc_js( $plugin_safe_name ).'.md') ) {
			//echo esc_js( $plugin_safe_name );
			...
			$current = file_get_contents($file);
				...
				//~ //	echo "<pre>";
				    ...
				    "Found matches:\n";
				    $core_string = implode("\n", $matches[0]);
					$act_string = explode(' ',$core_string);
					$the_output = '';
					$string = $core_string;
					$start = '(';
					$end = ')';
					...
					$oooutput = explode(' ',substr($string, $ini, $len));
					 $the_fetched_color = $oooutput[1];
			...
			$note_color_style = ( ( $note_color !== $this->defaultcolor ) ? ' style="background-color: ' . $the_fetched_color . ';"' : '' );
			...
			<div id="wp-plugin_note_' . esc_attr( $plugin_safe_name ) . '" ondblclick="edit_plugin_note(\'' . esc_js( $plugin_safe_name ) . '\');" title="' . __('Double click to edit me!', 'plugin-notes') . '">
				<span class="wp-plugin_note mynot'.esc_attr( $plugin_safe_name ).'">' . $filtered_note_text . '</span>
		...
		if(!empty($note_color)){
						$output .= '<span class="wp-plugin_note_status" > Status : '.$note_color .'</span>';
					}
...
'					<a href="#" onclick="save_plugin_note(\'' . esc_js( $plugin_safe_name ) . '\');return false;" class="button-primary">' . __('Save', 'plugin-notes') . '</a>
		...
		function ajax_edit_plugin_note ( ) {
		    ...
		    $file=WP_PLUGIN_DIR .'/'. esc_js( $plugin_name ).'.md';
			//$files = "plugin-slug-readme.md";
			
		
			if(file_exists(WP_PLUGIN_DIR . '/'.esc_js( $plugin_name ).'.md') ) {
			//echo esc_js( $plugin_safe_name );
